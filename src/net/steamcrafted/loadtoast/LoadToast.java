...
public class LoadToast {
	...
	public LoadToast(Context context) {
		...
		mParentView.postDelayed(new Runnable() {
			@Override
			public void run() {
				ViewHelper.setTranslationX(mView,
						(mParentView.getWidth() - mView.getWidth()) / 2);
				ViewHelper.setTranslationY(mView, -mView.getHeight()
						+ mTranslationY);
				mInflated = true;
	...
	public LoadToast show() {
		...
		ViewPropertyAnimator.animate(mView).alpha(1f)
