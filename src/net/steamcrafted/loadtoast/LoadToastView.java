...
public class LoadToastView extends View {
	...
	public LoadToastView(Context context) {
		...
		completeicon = getResources().getDrawable(
	...
	private void calculateBounds() {
		...
		if (mTextBounds.width() > MAX_TEXT_WIDTH) {
			...
			while (textSize > dpToPx(13)
					&& mTextBounds.width() > MAX_TEXT_WIDTH) {
				textSize--;
				// Log.d("bounds", "width " + mTextBounds.width() + " max " +
				// MAX_TEXT_WIDTH);
			...
			if (mTextBounds.width() > MAX_TEXT_WIDTH) {
				outOfBounds = true;
				/**
				 * float keep = (float)MAX_TEXT_WIDTH /
				 * (float)mTextBounds.width(); int charcount =
				 * (int)(mText.length() * keep); //Log.d("calc", "keep " +
				 * charcount + " per " + keep + " len " + mText.length()); mText
				 * = mText.substring(0, charcount);
				 * textPaint.getTextBounds(mText, 0, mText.length(),
				 * mTextBounds);
				 **/
			}
		}
	...
	protected void onDraw(Canvas c) {
		...
		spinnerRect.set(iconBounds.left + dpToPx(4) - translateLoad / 2,
				iconBounds.top + dpToPx(4), iconBounds.right - dpToPx(4)
						- translateLoad / 2, iconBounds.bottom - dpToPx(4));
		...
		toastPath.rCubicTo(circleOffset, 0, th / 2, th / 2 - circleOffset,
				th / 2, th / 2);

		toastPath.rLineTo(-pd, 0);
		toastPath.rCubicTo(0, -iconoffset, -iw / 2 + iconoffset, -iw / 2,
				-iw / 2, -iw / 2);
		toastPath.rCubicTo(-iconoffset, 0, -iw / 2, iw / 2 - iconoffset,
				-iw / 2, iw / 2);
		toastPath.rCubicTo(0, iconoffset, iw / 2 - iconoffset, iw / 2, iw / 2,
				iw / 2);
		toastPath.rCubicTo(iconoffset, 0, iw / 2, -iw / 2 + iconoffset, iw / 2,
				-iw / 2);
		toastPath.rLineTo(pd, 0);

		toastPath.rCubicTo(0, circleOffset, circleOffset - th / 2, th / 2,
				-th / 2, th / 2);
		toastPath.rLineTo(ws * (-IMAGE_WIDTH - MAX_TEXT_WIDTH), 0);
		toastPath.rCubicTo(-circleOffset, 0, -th / 2, -th / 2 + circleOffset,
				-th / 2, -th / 2);
		...
		c.drawCircle(spinnerRect.centerX(), spinnerRect.centerY(),
				iconBounds.height() / 1.9f, backPaint);
		// loadicon.draw(c);
		...
		toastPath.arcTo(spinnerRect, 180 * progrot, Math.min((200 / .75f)
				* proglength + 1 + 560 * (1f - ws), 359.9999f));
		...
		if (WIDTH_SCALE > 1f) {
			...
			int paddingicon = (int) ((1f - (.25f + (.75f * circleProg)))
					* TOAST_HEIGHT / 2);
			...
			c.drawCircle(leftMargin + TOAST_HEIGHT / 2, (1f - circleProg)
					* TOAST_HEIGHT / 8 + TOAST_HEIGHT / 2,
					(.25f + (.75f * circleProg)) * TOAST_HEIGHT / 2,
					(success) ? successPaint : errorPaint);
			c.save();
			c.rotate(90 * (1f - circleProg), leftMargin + TOAST_HEIGHT / 2,
					TOAST_HEIGHT / 2);
			icon.draw(c);
			c.restore();
		...
		if (outOfBounds) {
			...
			if (prevUpdate == 0) {
				...
				shift = ((System.currentTimeMillis() - prevUpdate) / 16f)
						* MARQUE_STEP;
			...
			c.drawText(mText, 0, mText.length(), th / 2
					+ (MAX_TEXT_WIDTH - mTextBounds.width()) / 2, yPos,
					textPaint);
		}
		// c.drawArc(spinnerRect, 360 * progrot, 200 * proglength + 1, true,
		// loaderPaint);
	}
