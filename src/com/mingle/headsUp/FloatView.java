...
public class FloatView extends LinearLayout {
	 ...
	 * 
	 * @param event
	 *            鍚慥elocityTracker娣诲姞MotionEvent
	 * 
	...
	public void translationX(float fromX, float toX, float formAlpha,
			final float toAlpha) {
		ObjectAnimator a1 = ObjectAnimator.ofFloat(rootView, "alpha",
				formAlpha, toAlpha);
		ObjectAnimator a2 = ObjectAnimator.ofFloat(rootView, "translationX",
				fromX, toX);
	...
	public void setNotification(final HeadsUp headsUp) {
		...
		mHandle = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if (headsUp.isActivateStatusBar()) {
					HeadsUpManager.getInstant(getContext()).silencerNotify(
							headsUp);
				}
		...
		if (headsUp.getCustomView() == null) {
			...
			Drawable iconDrawable = headsUp.getIconDrawable();

			View defaultView = LayoutInflater.from(getContext()).inflate(
					R.layout.notification, rootView, false);
			rootView.addView(defaultView);
			ImageView imageView = (ImageView) defaultView
					.findViewById(R.id.iconIM);
			TextView titleTV = (TextView) defaultView
					.findViewById(R.id.titleTV);
			TextView timeTV = (TextView) defaultView.findViewById(R.id.timeTV);
			TextView messageTV = (TextView) defaultView
...
//				getTitleTextColor(): 返回一个适合做标题颜色的颜色值
				Palette palette = Palette.generate(((BitmapDrawable)iconDrawable).getBitmap());
//				Logger.d("Swatches Size", Integer.toString(palette.getSwatches().size()));
//				for (Swatch wSwatch : palette.getSwatches()) {
//					Logger.d("Swatche ", wSwatch==null?"空":Integer.toString(wSwatch.getBodyTextColor()));
//				}
				Palette.Swatch swatch = palette.getSwatches().get(0);
				if (swatch != null) {
                    //view.setBackgroundColor(swatch.getRgb());
//    				titleTV.setTextColor(swatch.getTitleTextColor());
//    				messageTV.setTextColor(swatch.getBodyTextColor());
                } 
			}else{
			...
			if (headsUp.isExpand() && headsUp.getActions().size() > 0) {
				...
				ImageView imageView1 = (ImageView) defaultView
						.findViewById(R.id.menuIM1);
				ImageView imageView2 = (ImageView) defaultView
						.findViewById(R.id.menuIM2);
				ImageView imageView3 = (ImageView) defaultView
						.findViewById(R.id.menuIM3);
				TextView text1 = (TextView) defaultView
						.findViewById(R.id.menuText1);
				TextView text2 = (TextView) defaultView
						.findViewById(R.id.menuText2);
				TextView text3 = (TextView) defaultView
						.findViewById(R.id.menuText3);
				imageView1.setImageResource(headsUp.getActions().get(0).icon);
				text1.setText(headsUp.getActions().get(0).title);

				defaultView.findViewById(R.id.menu1).setOnClickListener(
						new OnClickListener() {
							@Override
							public void onClick(View v) {
								try {
									headsUp.getActions().get(0).actionIntent
											.send();
									cancel();
								} catch (PendingIntent.CanceledException e) {
									e.printStackTrace();
				...
				if (headsUp.getActions().size() > 1) {
					...
					imageView2
							.setImageResource(headsUp.getActions().get(1).icon);
					text2.setText(headsUp.getActions().get(1).title);
					defaultView.findViewById(R.id.menu2).setOnClickListener(
							new OnClickListener() {
								@Override
								public void onClick(View v) {
									try {
										headsUp.getActions().get(1).actionIntent
												.send();
										cancel();
									} catch (PendingIntent.CanceledException e) {
										e.printStackTrace();
				...
				if (headsUp.getActions().size() > 2) {
					...
					imageView3
							.setImageResource(headsUp.getActions().get(2).icon);
					text3.setText(headsUp.getActions().get(2).title);
					defaultView.findViewById(R.id.menu3).setOnClickListener(
							new OnClickListener() {
								@Override
								public void onClick(View v) {
									try {
										headsUp.getActions().get(2).actionIntent
												.send();
										cancel();
									} catch (PendingIntent.CanceledException e) {
										e.printStackTrace();
